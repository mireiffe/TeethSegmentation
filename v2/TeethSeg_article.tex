...
\end{align*}
and $\delta_{S}(X)$ is a dilation of a set $X$ by a structuring element $S$ which is a $3\times3$ matrix with origin at its center. Observe that the initial contour goes outward in $\Omega_b = \Omega\setminus\Omega_a$ by a balloon force $F_b$ and approaches the edge by $F_a$ once it reaches in $\Omega_a$.
