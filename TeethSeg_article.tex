...
Segmentation of individual tooth in human teeth images have mainly been performed in computed tomography (CT) or X-ray images \cite{Yuan:2020:teethParanomic,Naumovich:2015:teethCT3D,Said:2006:teethxray,Shah:2006:teethautoseg}. Tooth regions are easily segmented in these CT or X-ray images since the intensities of teeth region are much stronger than the gums or tissues. However, segmenting individual tooth in a teeth image taken outside the mouth with a camera is a more difficult problem. In the teeth images, not only weak edges and light reflections appear but also the colors and illuminance are inhomogeneous because of plaques and the oral structure, and these are main obstacles that make individual tooth segmentation difficult. Several attempts have been made to segment individual tooth in a teeth image. In \cite{LeeWater:2010,Na:2014LteethMorph} the watershed algorithm with color information was proposed. In \cite{LeeSegRemoval:2010}, an additional light reflection removal process \cite{LeeRemoval:2010} was applied before segmentation. In recent studies \cite{Kim:2020,Pham:2020,Zhu:2020:teethMaskrcnn}, segmentations using deep learning methods based on Mask R--CNN \cite{He:2018:MRCNN} were proposed. The main application area is 3D tooth model reconstruction from 2D tooth images. The 3D teeth model reconstruction have been developed based on individual tooth segmentation in CT images \cite{Yuan:2020:teethParanomic,Naumovich:2015:teethCT3D}. But in a recent work \cite{WU:2016}, a 3D teeth model reconstruction method using individual tooth boundaries in a single teeth image was proposed, which suggests the importance of segmentation of tooth regions.

Existing methodologies can be divided into two categories; model--based method \cite{LeeWater:2010,LeeSegRemoval:2010,Na:2014LteethMorph} or supervised learning method \cite{Kim:2020,Pham:2020,WU:2016,Zhu:2020:teethMaskrcnn}. Model--based methods used the watershed algorithm \cite{Vincent:1991:watershed} based on the color information of image. Although these methods are reliable, produces poor results when applied to images obtained in an unrefined environment. On the other hand, it is well known that a supervised learning methods can give outstanding results for a variety of images but sufficient amount of training data is required for stable performance, and this labeling process consumes a lot of time and effort. 
% In \cite{WU:2016}, a supervised learning method BEL \cite{Dollar:2006:BEL} was used to obtain the individual tooth boundaries, and manually labeled $40$ teeth studio-taken images are used as the training data.
...
The remainder of this paper is organized as follows. In Section~\ref{Sec:er_teeth}, we will talk about geometric attraction-driven flow (GADF), edge regions, and the problems of edge regions obtained from teeth images. In Section~\ref{Sec:PM}, the proposed algorithm is described and the experimental results are present in Section~\ref{Sec:result}. We will conclude this paper at Section~\ref{Sec:Conclusion}. 

% Sec:Background
\section{Edge regions in human teeth image}
\label{Sec:er_teeth}
In this section, we briefly review GADF \cite{Hahn:2006, Hahn:2010:GADF} and edge region, which play key role in segmenting individual tooth. Concerning the problematic features of the teeth image we will show weak points of edge region obtained from GADF.
...
\begin{align}
    ...
    \phi(x,\,0) &= \phi_0(x)\cm \nonumber
\end{align}
where $\mu$ is a constant, $\phi_0$ is the initial signed distance function (SDF), $\chi$ is a characteristic function of the set of subscription and $\oma$ is a region in which $F_a$ faces each other, i.e.,
\begin{align}
...
\end{align*}
and $\delta_{S}(X)$ is a dilation of a set $X$ by a structuring element $S$ which is a $3\times3$ matrix with origin at the center. Observe that the initial contour goes outward in $\Omega_b = \Omega\setminus\Omega_a$ by a balloon force $F_b$ and approaches the edge by $F_a$ once it reaches in $\Omega_a$.

The edge region is a region that is thought to contain edges. Since $\oma$ contains all edges by the definition, it can be a first candidate of the edge region, and in fact, whole region of $\oma$ is used as an edge region in \eqref{Eq:gadf}. But, in practice $\oma$ contains a lot of unnecessary parts, and so it cannot be used as an edge region for the individual tooth segmentation.

% Subsection: Edges outside the tooth boundaries
\subsection{Edges beside the tooth boundaries}
\label{Subsec:er_outside}

As shown in Figure~\ref{Fig:1d}, $\oma$ contains all existing edges in the image and so it contains many parts beside the tooth boundaries.  
...
\begin{figure}
    ...
    \begin{subfigure}{.165\textwidth}
        ...
        \label{Fig:1d}
    \end{subfigure}
    \caption{(a) A teeth image and (b) sub-images in the green boxes. The results of different methods are shown. (c--d) Results of Sobel and Canny edge detectors with automatically chosen thresholds by MATLAB. (e) $\oma$ from the GADF. Weak edges are appeared at the boundary between teeth and gums, the boundary between two teeth with similar color and brightness, and the junction of boundaries. As shown in the results, GADF can detect tooth boundaries much better than the other methods while there are many unnecessary parts. }
