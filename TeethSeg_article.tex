...
If the pseudo edge-region perfectly matches the tooth boundaries, all tooth regions are surrounded by thin regions, so initial contours can be naturally selected as the boundary of the thin regions. However, there are often leakages in the pseudo edge-region, which does not enclose the tooth region. Thus, multiple objects may be linked into a single region, in which case each object must be separated. Let $\mathcal{S}$ be such a single region. By taking all local minima of $\sdf(\mathcal{S})$, we can retain landmarks for linked objects in $\sdf(\mathcal{S})$; see the third row of Figure~\ref{Fig:pre_er}. 
...
\end{align}
where $\mu$, $\nu$ are constants, $\kappa(\phi_i)$ is the curvature of $\phi_i$, $\PER$ is the pseudo edge-region, and $\phi_i^L = \sdf(L_i)$ for all $i=1,\,\ldots,\,K$. The first term is a regularization term, and the second term makes contour $\phi_i$ stop when it meets the zero level sets of other $\phi_j$ including $\phi_0$. The third term prevent the contour to enter $\PER$. In order to resolve the over-segmentation, a standby process is introduced when applying the evolution~\eqref{Eq:evolv_init}; until all contours reach a certain range of $\PER$, contours that touch each other are merged rather than stop by $\fci$. In this process, contour points that have already reached a certain range of $\PER$ stop and wait until all the contours reach the range. When all contours reach the range, they evolve again. Contours in standby and resulting contours are shown in the last two rows of Figure~\ref{Fig:pre_er}. We take these resulting contours as the initial contours for the contour evolution for segmentation.

\subsubsection{Segmentation using active contours}
\label{Subsec:segmentation}

Now, we segment each region using multiple contours found in the previous section. Let $\left\{\phi_{i,\,0}\right\}_{i=1}^{N}$ be a collection of level set functions which are steady-state solutions of~\eqref{Eq:evolv_init}. Here, we assume that $\PER$ contains the tooth boundaries and let $\ER$ be edge-region candidate intersecting $\PER$, i.e., $\ER=\PER\cap\ome$. Since the contours touch the $\partial\PER$, we push $\phi_{i,\,0}$ outward until it reaches the boundary of the tooth. For this, we propose a contour evolution with a level set formulation:
\begin{align}
    \begin{split}
