...
\end{align}
as a pseudo edge-region of $X$, and examples of pseudo edge-regions for teeth images are shown in Figure~\ref{Fig:pre_er}. One notable point is that the pseudo edge-regions obtained from the neural network look better than the regions obtained from the manual selection, i.e. the labels.
...
\begin{figure}[]
    ...
    \end{subfigure}
    \caption{First row: Input images. Second row: Pseudo edge-region from the neural network. }
    \label{Fig:pre_er}
\end{figure}

% Subsection: Acquiring initial contours
\subsection{Acquiring initial contours}
\label{Subsec:refine}
...
If the pseudo edge-region is perfectly given, then all individual tooth regions are enclosed by curves consisting the pseudo edge-region, so that initial contour for each individual tooth region is naturally given by the boundary of the enclosed region. However, pseudo edge-region contains small fragments and leakages, which mean breakage of the closed curve, thus it is required to find positions for each individual tooth region. 

First, the non-tooth regions are identified among the enclosed regions. The methods listed below are criterions for identifying non-tooth regions. For a enclosed region $X$, let $c$ be the center of mass and $\mathcal{I}$ the 2-D moment of inertia tensor about $c$ defined by
\begin{align*}
    \mathcal{I} &=
    \begin{bmatrix}
        I_{xx} & I_{xy}\\
    ...
    \end{bmatrix}\cm\\
\end{align*}
where $x$, $y$ are coordinates of the vector from $c$ to each point of $X$, and
\begin{align*}
    I_{xx} = \int_{X} my^2 \cm\quad I_{yy} = \int_{X} mx^2\quad\mbox{and}\quad I_{xy} = I_{yx} = -\int_{X} mxy\pd
\end{align*}
Then the inertia tensor has two eigenvalues $\Lambda$ and $\lambda$ ($\Lambda > \lambda$). Let $v$ be a normalized eigenvector corresponding to $\lambda$.
\begin{itemize}
    ...
    \item [(C--3)] Count the number of points with positive or negative curvature on the boundary of region. If
    \begin{align*}
        \int_{\partial{X}} \sgn(\kappa(s)) \operatorname{d}\!s < -0.2 \int_{\partial{X}} \operatorname{d}\!s\cm
    \end{align*}
