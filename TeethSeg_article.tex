...
In the initial-state, distances from each contour to $\PER$ are all different. In order to prevent leakage through gaps of $\PER$ and ensure that all contours reach $\PER$ at the same time, a stand-by process is introduced when applying the evolution~\eqref{Eq:evolv_init}: While a contour $\mathcal{C}$ evolves, the values of $\sdf(\PER)$ on $\mathcal{C}$ are decreasing. A part of $\mathcal{C}$ stops and waits when $\sdf(\PER)$ on that part is less then or equal to a certain value. Once $\sdf(\PER)$ on every contour is less than or equal to that value, they continue to evolve again. During this process, the contours touch each other are merged without stopping by $\fci$ to avoid over-segmentation. Standing-by and resulting contours are shown in the last two rows of Figure~\ref{Fig:pre_er}.
...
In human teeth image with $R$, $G$ and $B$ channels, it has been reported that the values of $R$ and $G$ channels in teeth are relatively similar compared to other parts because of the white or ivory color of human teeth.~\cite{LeeRemoval:2010}. Based on this characteristics of human teeth, we propose the teeth enhanced map $G/R-\alpha B$ for a teeth image; the value of $G/R$ is close to $1$ in teeth and close to $0$ in other reddish parts such as gums and lips. However, $G/R$ emphasizes not only teeth but also light reflection. Because light reflection is usually whiter than teeth, it has the larger $B$ value than teeth. Thus, by subtracting a multiple of $B$ from $G/R$, the emphasis of the light reflection can be reduced. For the regions segmented in Section~\ref{Subsec:SegLevelset}, if the tooth enhanced map is less than the average in more than $25\%$ of a region, the region is remove. Examples of the teeth enhanced map are shown in Figure~\ref{Fig:tem}.
