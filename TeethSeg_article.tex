...
\end{align}
as a pseudo edge-region of $X$, and examples of pseudo edge-regions for teeth images are shown in the first two rows of Figure~\ref{Fig:pre_er}. One notable point is that the pseudo edge-region obtained by the neural network look better than the regions obtained by the manual selection from $\ome$.
...
In this section, we propose two active contour models formulated by the level set method, one for acquiring initial contours and the other for segment regions. We denote $\sdf(A)$ as the SDF which is negative in the set $A$ and constantly zero on $\partial A$.

\subsubsection{Acquiring initial contours}
\label{Subsec:initial_contours}

If the pseudo edge-region perfectly matches the tooth boundaries, all tooth regions are surrounded by thin regions, so initial contours can be naturally selected as the boundary of the thin regions. However, there are often leakages in the pseudo edge-region, which does not enclose the tooth region. Thus, multiple objects may be linked into a single region, in which case each object must be separated. When multiple objects are linked, the shape or orientation of the region distinguishes from the single tooth region. Hence, we first consider how to distinguish the tooth region from the other regions. 

For a connected region $X$, let $c$ be the center of mass and $\mathcal{I}$ be the 2 dimensional moment tensor about $c$ defined by
\begin{align*}
    \mathcal{I} &=
    \begin{bmatrix}
        I_{xx} & I_{xy}\\
    ...
    \end{bmatrix}\cm
\end{align*}
where $x$ and $y$ are coordinates of the vector from $c$ to each point of $X$, and
\begin{align*}
    I_{xx} = \int_{X} x^2 \cm\quad I_{yy} = \int_{X} y^2\quad\mbox{and}\quad I_{xy} = I_{yx} = \int_{X} xy\pd
\end{align*}
Then the moment tensor $I$ has two eigenvalues $\Lambda$ and $\lambda$ ($\Lambda > \lambda$). Let $v$ be a normalized eigenvector corresponding to $\lambda$. The methods listed below are the criteria for identifying non-tooth regions: 
\begin{itemize}
    \setlength{\itemindent}{2mm}
    \item [(C--1)] For the curvature $\kappa(s)$ at $s\in\partial{X}$, 
    \begin{align*}
        \frac{1}{|\partial{X}|}\int_{\partial{X}} \mbox{sgn}(\kappa(s)) ds < 0.2\pd
    \end{align*}

    \item [(C--2)] The center of mass $c$ is outside the region.

    \item [(C--3)] The ratio $\Lambda / \lambda \ge 2.5$ and $|v\cdot e_1| \ge \cos(\pi/4)$ for $e_1 = (1,\,0)$.
    
\end{itemize}
If one of the above criteria is satisfied, the region is identified as not a tooth region. Otherwise, the region is regarded as a tooth. Let $\omt$ and $\omn$ be tooth and non-tooth regions, respectively, identified by the above criteria. For $\omn$, at all local minima of $\sdf(\omn)$, we can retain landmarks for each object in the region; see the third row of Figure~\ref{Fig:pre_er}. 

In order to get better initial contours, each landmark is expanded to touch the pseudo edge-region. Let $\{L_i\}_{i=1}^K$ be distinct $K$ landmarks. For each level set function $\phi_i$, the competing balloon force $\fci$ is defined as
\begin{align}
    \fci(x,\,t) =
...
\end{align}
where $\mu$, $\nu$ are constants, $\kappa(\phi_i)$ is the curvature of $\phi_i$, $\PER$ is the pseudo edge-region, and $\phi_i^L = \sdf(L_i)$ for all $i=1,\,\ldots,\,K$. The first term is a regularization term, and the second term makes contour $\phi_i$ stop when it meets the zero level sets of other $\phi_j$ including $\phi_0$. The third term prevent the contour to enter $\PER$. In order to resolve the over-segmentation, a standby process is introduced when applying the evolution~\eqref{Eq:evolv_init}; until all contours touch $\PER$ within a certain range, contours that touch each other are merged rather than stop by $\fci$. For $\omt$, we take the contour as $\partial\omt$. Resulting contours are shown in the last row of Figure~\ref{Fig:pre_er}. We take these contours as the initial contours for the contour evolution for segmentation.

\subsubsection{Segmentation using active contours}
\label{Subsec:segmentation}

Now, we segment each region using multiple contours found in the previous section. Let $\left\{\phi_{i,\,0}\right\}_{i=1}^{N}$ be a collection of level set functions which are steady-state solutions of~\eqref{Eq:evolv_init}. Here, we assume that $\PER$ contains the tooth boundaries. Since the contours touch the $\partial\PER$, we push $\phi_{i,\,0}$ outward until it reaches the boundary of the tooth. For this, we propose a contour evolution with a level set formulation:
\begin{align}
    \begin{split}
...
As a final step, the identification of the segmented regions remains. Since our goal is to segment each individual tooth, we need to identify tooth and non-tooth regions from the result in Section~\ref{Subsec:SegLevelset}. In a human teeth image, tooth and non-tooth regions can be easily distinguished by shape and color. Teeth are relatively white and convex in shape, thus non-tooth regions can be easily removed. 
