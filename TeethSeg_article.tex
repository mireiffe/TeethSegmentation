...
The edge-region is roughly defined as the curve of thin regions containing most of object boundaries, regardless of the strength of edges. In~\cite{Hahn:2010:GADF}, the region $\ome$, called the candidate of edge-regions, is defined as 
\begin{align*}
    \ome = \left\{ x\in\Omega \mid F_a(x^*) \cdot F_a(x) < 0 ~\mbox{and}~ x^*=x+F_a(x) \right\}\pd %\label{Def:pre_er}
\end{align*}
Since GADFs are vector field of normalized gradients designed toward edge points, $\ome$ contains all object boundaries, regardless of the strength of edges. 

In the paper~\cite{Hahn:2010:GADF}, $\oma=\delta_S(\ome)$ is used as the edge-region, and because of the characteristics of omee, it fits well with the rough definition of the boundary region.

이 논문에서는 ome를 dilate한 omee를 경계영역으로 사용하고 있는데, omee의 특징때문에 이 것은 경계영역에 대한 러프한 정의에 잘 부합한다. 하지만, 실제 이미지들은 물체와 물체 사이 뿐만 아니라, 잡티와 빛반사 들 여러 요소들로 인해 생기는 잡다한 경계들이 존재하고 있다. 이 말은, $\oma$ is just a candidate for the edge-region, because $\ome$ contains many regions that are not intersected with the boundaries of the objects. Hence it is necessary to remove them as many as possible. 이 논문에는, 이미지의 인텐시티와 omee의 형태적 특성을 고려하여 필요없는 부분을 제거하려는 아이디어를 사용하였다. 그림 2에는 깔끔하게 정돈된 이미지와 프렉티컬한 이미지에서 계산된 omee를 각각 확인할 수 있다. 정돈된 이미지에 대한 omee는 물체간의 경계에서만 잘 형성되는 경계영역이 나타나지만, 실제의 이미지에서는 그 외에도 아주 많은 잡다한 영역이 함께 형성되는 걸 볼 수 있으며, 논문과 같은 방식으로 필요없는 영역을 제거한 결과도 함께 볼 수 있다.


The edge-region $\delta_S(\ome)$ used in~\cite{Hahn:2010:GADF} is appropriate to the rough definition:   is used as the edge-region where
and $\delta_{S}(X)$ is a dilation of a set $X$ by a $3 \times 3$ square structuring element $S$ with origin at its center. Here, $\ome$ is the region where GADF faces each other, and since the GADF


In practice, as shown in Figure~\ref{Fig:1d}, $\oma$ is a candidate for the edge-region, because $\ome$ contains many regions that are not intersected with the boundaries of the objects. Hence it is necessary to remove them as many as possible.
