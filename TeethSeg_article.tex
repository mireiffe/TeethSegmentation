...
As shown in Figure~\ref{Fig:flowchart}(a), we make labels for training data using GADF and manual selection. First, we manually select the connected components of $\ome$ intersecting the tooth boundary. The label $Y\colon\Omega\subset\mathbf{R}^2\rightarrow\{0,\,1\}$ is defined as a binary image which is $1$ on the dilation of the selected regions by $\eta\times\eta$ structuring element and $0$ otherwise. In Figure~\ref{Fig:labeling}, an example of the labeling process is presented.
